...
Поки шаблон не вставлено, Supabase шле свій дефолтний лист: сірий текст
без бренду, а посилання в ньому веде на Site URL, тобто на лендінг. Саме
тому підтвердження «викидало на сайт».
...
## Куди веде кожен лист

- Confirm signup, `/auth/callback` → `/admin/login?confirmed=1`: після
  підтвердження людина бачить форму входу з зеленим «Пошту підтверджено»,
  а не лендінг і не одразу кабінет (живий фідбек).
- Reset Password → `/reset-password`.
- Change Email Address, `/auth/callback` → `/admin/profile`.

Мертве або вже використане посилання в перших двох випадках веде на
`/admin/login?error=auth`, і сторінка входу це пояснює.
